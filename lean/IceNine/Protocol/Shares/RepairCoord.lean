...
-- HasSender instances for repair messages
instance {S : Scheme} : HasSender (ContribCommitMsg S) S.PartyId where
  sender := fun m => m.sender

instance {S : Scheme} : HasSender (ContribRevealMsg S) S.PartyId where
  sender := fun m => m.sender
...
abbrev contribCommitSender {S : Scheme} : ContribCommitMsg S → S.PartyId := getSender

/-- Sender accessor for contribution reveal messages. -/
abbrev contribRevealSender {S : Scheme} : ContribRevealMsg S → S.PartyId := getSender
...
def RepairCoordState.enoughCommits (S : Scheme) [BEq S.PartyId] [Hashable S.PartyId]
    (st : RepairCoordState S) : Bool :=
  st.commits.size ≥ st.threshold

/-- Check if all committed helpers have revealed. -/
def RepairCoordState.allRevealed (S : Scheme) [BEq S.PartyId] [Hashable S.PartyId]
    (st : RepairCoordState S) : Bool :=
...
def processContribReveal (S : Scheme) [BEq S.PartyId] [Hashable S.PartyId] [DecidableEq S.Commitment]
  ...
  if st.phase = .reveal then
    ...
    | some c =>
        if verifySecretCommitment S msg.contribution msg.opening c.contribCommit then
...
/-- Verify the repaired share against known public share.
    A(sk_i) must equal pk_i. Uses generic share verification. -/
def verifyRepairedShareCoord (S : Scheme) [DecidableEq S.Public]
    (repairedSk : S.Secret) (expectedPk : S.Public) : Bool :=
  verifySecretPublicBool S repairedSk expectedPk
...
def lagrangeCoefficient {F : Type*} [Field F] [DecidableEq F]
    (partyScalar : F)
    (allScalars : List F)
    : F :=
...
def allLagrangeCoeffs {F : Type*} [Field F] [DecidableEq F]
