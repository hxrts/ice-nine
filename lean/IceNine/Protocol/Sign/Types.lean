...
/-- EvidenceCarrier instance for SignCommitMsg. -/
instance (S : Scheme) : EvidenceCarrier (SignCommitMsg S) where
...
/-- EvidenceCarrier instance for SignRevealWMsg. -/
instance (S : Scheme) : EvidenceCarrier (SignRevealWMsg S) where
  attachEvidence := fun msg evid =>
    { msg with context := { msg.context with evidenceDelta := some evid } }
  extractEvidence := fun msg => msg.context.evidenceDelta
...
structure Signature (S : Scheme) where
  ...
  /-- External context (merged from contributing shares) -/
  context : ExternalContext := {}

/-- EvidenceCarrier instance for Signature. -/
instance (S : Scheme) : EvidenceCarrier (Signature S) where
  attachEvidence := fun sig evid =>
...
For protocols like Aura's fallback gossip, shares need to be verifiable
independently of full session state. A ValidatableShare bundles a share
with enough context for independent validation.
-/

/-- Extended share message with validation context.
    Contains enough information to verify the share without full session state.

    **Use case**: Aura fallback gossip needs to validate shares received
    from peers without having participated in the original signing session. -/
structure ValidatableShare (S : Scheme) where
  /-- The core share data -/
  share : SignShareMsg S
  /-- Public nonce commitment (for binding verification) -/
  nonceCommit : S.Commitment
  /-- Public hiding commitment W_hiding = A(hiding nonce) -/
  publicHiding : S.Public
  /-- Public binding commitment W_binding = A(binding nonce) -/
  publicBinding : S.Public
  /-- Public key share of sender (pk_i = A(sk_i)) -/
  pkShare : S.Public

/-- Create a ValidatableShare from signing session data. -/
def ValidatableShare.create (S : Scheme)
    (share : SignShareMsg S)
    (commit : SignCommitMsg S)
    (pkShare : S.Public)
    : ValidatableShare S :=
  { share := share
    nonceCommit := commit.commitW
    publicHiding := commit.hidingVal
    publicBinding := commit.bindingVal
    pkShare := pkShare }

/-- Access the sender from a ValidatableShare. -/
def ValidatableShare.sender {S : Scheme} (vs : ValidatableShare S) : S.PartyId :=
  vs.share.sender

/-- Access the session from a ValidatableShare. -/
def ValidatableShare.session {S : Scheme} (vs : ValidatableShare S) : Nat :=
  vs.share.session
...
instance (S : Scheme) : EvidenceCarrier (ValidatableShare S) where
  attachEvidence := fun vs evid =>
    { vs with share := EvidenceCarrier.attachEvidence vs.share evid }
  extractEvidence := fun vs => EvidenceCarrier.extractEvidence vs.share

/-!
## Context-Aware Aggregation

Aggregation functions that validate external context consistency across shares.
For protocols like Aura, this ensures all shares reference the same consensus instance.
-/

/-- Result of context validation across shares. -/
inductive ContextValidationResult
  | ok
      -- All contexts are consistent
  | inconsistentConsensusId (party1 party2 : Nat)
      -- Different consensusIds found at indices party1, party2
  | inconsistentResultId (party1 party2 : Nat)
      -- Different resultIds found at indices party1, party2
  | inconsistentPrestateHash (party1 party2 : Nat)
      -- Different prestateHashes found at indices party1, party2
  deriving DecidableEq, Repr
  ...
  | first :: rest =>
      let checkPair := fun (idx : Nat) (sh : SignShareMsg S) =>
        if !ExternalContext.isConsistent first.context sh.context then
          -- Determine which field is inconsistent
...
def aggregateWithContextValidation [BEq ByteArray]
    (S : Scheme)
    (c : S.Challenge)
    (Sset : List S.PartyId)
    (commits : List S.Commitment)
  ...
  match validateShareContexts shares with
  | .ok =>
      let z : S.Secret := (shares.map (·.z_i)).sum
      let ctx := shares.foldl (fun acc sh => ExternalContext.merge acc sh.context) ExternalContext.empty
      .ok { z := z, c := c, Sset := Sset, commits := commits, context := ctx }
  | .inconsistentConsensusId i j =>
      .error s!"Shares {i} and {j} have different consensusId"
  | .inconsistentResultId i j =>
      .error s!"Shares {i} and {j} have different resultId"
  | .inconsistentPrestateHash i j =>
      .error s!"Shares {i} and {j} have different prestateHash"
