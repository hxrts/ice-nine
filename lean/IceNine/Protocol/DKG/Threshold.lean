  ...
  pairs.foldl
    (fun acc (c,r) =>
      ...
      if c.sender = r.sender then
        ...
        if S.commit r.pk_i r.opening = c.commitPk then acc
...
def dkgAggregateWithComplaints
  ...
  if commits.length = reveals.length then
    ...
    let fallbackId := (commits.head?.map (·.sender)).getD ((reveals.head?.map (·.sender)).getD default)
    Except.error [Complaint.missingReveal fallbackId]
...
def Complaint.accused {PartyId : Type*} : Complaint PartyId → PartyId
  | .openingMismatch p => p
  | .missingReveal p => p

/-- Get all parties accused in a list of complaints.
    Uses extractUnique from Core. -/
def accusedParties {PartyId : Type*} [DecidableEq PartyId]
    (complaints : List (Complaint PartyId)) : List PartyId :=
  extractUnique Complaint.accused complaints

/-- Remove faulty parties from participant list -/
def excludeFaulty {PartyId : Type*} [DecidableEq PartyId]
...
def hasThresholdQuorum {PartyId : Type*} [DecidableEq PartyId]
    (allParties : List PartyId)
...
def computeValidParties {PartyId : Type*} [DecidableEq PartyId]
    (allParties : List PartyId)
    (complaints : List (Complaint PartyId))
...
def filterValidReveals {S : Scheme} [DecidableEq S.PartyId]
    (reveals : List (DkgRevealMsg S))
...
def filterValidCommits {S : Scheme} [DecidableEq S.PartyId]
    (commits : List (DkgCommitMsg S))
...
def dkgLagrangeCoeff {PartyId : Type*} {F : Type*} [Field F] [DecidableEq F] [DecidableEq PartyId]
...
def dkgWithExclusion
    (S : Scheme) [Field S.Scalar] [DecidableEq S.Scalar] [DecidableEq S.PartyId] [DecidableEq S.Commitment]
    (pidToScalar : S.PartyId → S.Scalar)
